...
print("The softmax action function was use for the output layer because it converts the output into a probability distribution, allowing the model to predict the class with the highest probability for each input instance.") #5 points
...
print("The size of the first hidden layer is 300. None means the batch size is unspecified, allowing flexibility during training..\
The total number of parameters of the first hidden layer is 235,500, which refers to the number of connections and biases.") #8 points
model.summary()
...
print("An epoch is a single pass through the entire training dataset during model training. It is a full iteration over all samples in the training set.") #6 points
...
model.fit(x_train, y_train, epochs=20) #6 point
